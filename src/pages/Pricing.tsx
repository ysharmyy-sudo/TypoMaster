...
const Pricing = () => {
  ...
  const handlePayLifetime = async () => {
    if (!user) return navigate('/login');
    // No Razorpay key in frontend:
    // Use Razorpay Payment Link generated by backend and redirect the user.
    const res = await apiPost<{ success: boolean; url: string; paymentLinkId: string }>(
      '/api/payment/create-payment-link',
      { amount: 999, plan: 'lifetime' }
    );
          ...
          {plans.map((plan, i) => (
            <div 
              ...
              onClick={() => {
                ...
                if (plan.name === 'Pro Aspirant') return alert('Monthly plan is temporarily disabled.');
                if (plan.name === 'Lifetime Master') return void handlePayLifetime();
              }}
            >
              {plan.highlight && (
